package au.radar.core

import kotlinx.serialization.Serializable

/**
 * How loudly a single kind of threat is treated.
 *
 * Per-kind settings exist because the kinds are genuinely not equivalent. A
 * fixed camera is at a known point and you want a fixed distance of warning. A
 * police car is somewhere in an area, might be facing either way, and is worth
 * knowing about earlier and more insistently. Roadworks, most of the time, you
 * want on the map and out of your ears.
 */
@Serializable
data class KindSettings(
    val enabled: Boolean = true,
    /** False means a tone rather than a spoken warning. */
    val voice: Boolean = true,
    /** Pulse the screen, for warnings you might miss over loud audio. */
    val flash: Boolean = false,
    /** Seconds of warning on the open road. Converted to metres at your speed. */
    val leadSeconds: Double,
    /**
     * Warn about anything inside this radius whatever direction it lies in.
     *
     * This is the answer to the case heading alone cannot solve: something just
     * around a bend, on a side street you are about to cross, or on the far side
     * of a divided road. Within the radius we stop asking whether it is on your
     * road and simply tell you it is near.
     */
    val radiusM: Double,
)

/**
 * Everything the driver can tune, and the defaults they start from.
 *
 * The defaults are opinionated rather than neutral: police and mobile cameras
 * flash and get the longest lead, fixed cameras speak, congestion and roadworks
 * stay quiet. Someone who disagrees can change any of it.
 */
@Serializable
data class AlertSettings(
    val muted: Boolean = false,
    /** Master switch for the screen pulse, above the per-kind flags. */
    val flashEnabled: Boolean = true,
    /**
     * Below this, say nothing at all. Off by default — the parked rule already
     * covers the common case, and a driver crawling in traffic still wants to
     * know about the camera at the next set of lights.
     */
    val minSpeedKmh: Double = 0.0,
    /**
     * How much further to warn when the threat is on the road you are on. The
     * whole point of telling those two cases apart.
     */
    val sameRoadLeadMultiplier: Double = 1.7,
    /**
     * Half-width of the corridor that counts as "my road", at zero distance.
     * A touch under a wide divided carriageway.
     */
    val corridorHalfWidthM: Double = 40.0,
    /**
     * How much the corridor widens per metre of distance, absorbing heading
     * noise and gentle curves without swallowing the next street over.
     */
    val corridorWidenPerM: Double = 0.02,
    val corridorMaxHalfWidthM: Double = 90.0,
    val kinds: Map<String, KindSettings> = defaultKinds,
) {
    fun forKind(kind: String): KindSettings = kinds[kind] ?: fallbackKind

    companion object {
        /** Used for a kind nobody has configured, so a new feed cannot go silent. */
        val fallbackKind = KindSettings(leadSeconds = 20.0, radiusM = 300.0)

        /** One row per kind on the settings screen, in the order they are shown. */
        data class EditableKind(val kind: String, val title: String, val group: String)

        val editableKinds: List<EditableKind> = listOf(
            EditableKind("police", "Police", "Crowd reports"),
            EditableKind("mobile_camera", "Mobile camera (reported)", "Crowd reports"),
            EditableKind("mobile_zone", "Mobile camera zone", "Cameras"),
            EditableKind("trailer", "Camera trailer", "Cameras"),
            EditableKind("fixed_speed", "Fixed speed camera", "Cameras"),
            EditableKind("red_light_speed", "Red light and speed", "Cameras"),
            EditableKind("red_light", "Red light camera", "Cameras"),
            EditableKind("average_speed_start", "Average speed zone", "Cameras"),
            EditableKind("crash", "Crash", "Road conditions"),
            EditableKind("closure", "Road closed", "Road conditions"),
            EditableKind("flood", "Flooding", "Road conditions"),
            EditableKind("fire", "Fire", "Road conditions"),
            EditableKind("object_on_road", "Object on road", "Road conditions"),
            EditableKind("stopped_vehicle", "Stopped vehicle", "Road conditions"),
            EditableKind("hazard", "Other hazards", "Road conditions"),
            EditableKind("congestion", "Heavy traffic", "Quiet by default"),
            EditableKind("roadwork", "Roadworks", "Quiet by default"),
            EditableKind("event", "Events", "Quiet by default"),
        )

        val defaultKinds: Map<String, KindSettings> = mapOf(
            // The two people actually install this app for.
            "police" to KindSettings(
                voice = true, flash = true, leadSeconds = 45.0, radiusM = 500.0,
            ),
            "mobile_camera" to KindSettings(
                voice = true, flash = true, leadSeconds = 40.0, radiusM = 450.0,
            ),
            "mobile_zone" to KindSettings(
                voice = true, flash = true, leadSeconds = 35.0, radiusM = 400.0,
            ),
            "trailer" to KindSettings(
                voice = true, flash = false, leadSeconds = 30.0, radiusM = 350.0,
            ),

            // Fixed infrastructure: a known point, so a steady lead is enough.
            "fixed_speed" to KindSettings(leadSeconds = 25.0, radiusM = 300.0),
            "red_light" to KindSettings(leadSeconds = 22.0, radiusM = 250.0),
            "red_light_speed" to KindSettings(leadSeconds = 25.0, radiusM = 300.0),
            "average_speed_start" to KindSettings(leadSeconds = 30.0, radiusM = 350.0),
            "average_speed_end" to KindSettings(leadSeconds = 20.0, radiusM = 250.0),

            // Road conditions.
            "crash" to KindSettings(flash = true, leadSeconds = 35.0, radiusM = 400.0),
            "closure" to KindSettings(flash = true, leadSeconds = 45.0, radiusM = 500.0),
            "flood" to KindSettings(flash = true, leadSeconds = 40.0, radiusM = 450.0),
            "fire" to KindSettings(flash = true, leadSeconds = 40.0, radiusM = 450.0),
            "object_on_road" to KindSettings(leadSeconds = 30.0, radiusM = 350.0),
            "stopped_vehicle" to KindSettings(leadSeconds = 25.0, radiusM = 300.0),
            "hazard" to KindSettings(leadSeconds = 25.0, radiusM = 300.0),
            "alpine" to KindSettings(voice = false, leadSeconds = 30.0, radiusM = 400.0),

            // Things you can see for yourself. On the map, out of your ears.
            "congestion" to KindSettings(voice = false, leadSeconds = 20.0, radiusM = 250.0),
            "roadwork" to KindSettings(voice = false, leadSeconds = 15.0, radiusM = 200.0),
            "event" to KindSettings(voice = false, leadSeconds = 20.0, radiusM = 250.0),
        )
    }
}

/**
 * How a threat relates to the road you are on. This is the distinction the
 * whole engine turns on: something on your road deserves a much earlier warning
 * than something merely in front of you, and something very close deserves one
 * whichever way it lies.
 */
enum class Relation {
    /** On the route you are following, measured along the road, so curves count. */
    ON_ROUTE,

    /** Inside a narrow corridor straight down your heading: almost certainly your road. */
    SAME_ROAD,

    /** Within the forward cone, but off the corridor: a nearby road, or a bend. */
    AHEAD,

    /** Close enough that direction stops mattering. */
    NEARBY,
}

/**
 * The stretch of route immediately in front of the car, already trimmed by the
 * caller with [RouteTracker.aheadSlice].
 *
 * Trimming outside the engine keeps it cheap: projecting every nearby threat
 * onto a full cross-country polyline every second would be thousands of times
 * more work than projecting it onto the next few kilometres.
 */
@Serializable
data class RouteContext(val aheadGeometry: List<RoutePoint>)
